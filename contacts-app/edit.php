<?php
  ...
  $statement = $conn->prepare("SELECT * FROM dispositivos WHERE id = :id LIMIT 1");
  ...
  $dispostivo = $statement->fetch(PDO::FETCH_ASSOC);
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["nombre"]) || empty($_POST["link"])) {
      ...
      $statement = $conn->prepare("UPDATE dispositivos SET nombre = :nombre, link = :link WHERE id = :id");
      $statement->execute([
        ":id" => $id,
        ":nombre" => $_POST["nombre"],
        ":link" => $_POST["link"],
        ...
        <div class="col-md-8">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <?php endif ?>
              <form method="POST" action="edit.php?id=<?= $dispostivo["id"]?>">
                <div class="mb-3 row">
                  <label for="nombre" class="col-md-4 col-form-label text-md-end">Nombre</label>
    
                  <div class="col-md-6">
                    <input value="<?=$dispostivo["nombre"]?>"id="nombre" type="text" class="form-control" name="nombre" required autocomplete="nombre" autofocus>
                  </div>
                </div>
                ...
                <div class="mb-3 row">
                  ...
                  <div class="col-md-6">
                    <input value="<?=$dispostivo["link"]?>" id="link" type="tel" class="form-control" name="link" required autocomplete="link" autofocus>
                  </div>
                </div>
